...
1. 面向对象三大特性
   ...
   - **定义：** 将对象的状态（属性）和行为（方法）绑定在一起，并对外隐藏内部实现细节，只暴露必要的访问接口。
   - **Java体现：** `private` 属性 + `public` 的 getter/setter 方法。
   - 封装提高了代码的安全性和可维护性。

   继承：

   - **定义：** 继承是子类继承父类的属性和方法，使得代码可以复用，并建立类与类之间的层次结构。

   - 使用 `extends` 关键字实现继承。
   - 子类可以复用、重写（override）父类方法。
   - Java 中只支持单继承，但可以通过接口实现多继承的效果。
   - 增强了代码的可重用性和扩展性。

   抽象：

   - **定义：** 抽象是指隐藏具体的实现细节，只暴露对象的行为（接口或抽象方法），让使用者关注“做什么”，而不是“怎么做”。
   - 使用 `abstract` 关键字定义抽象类或抽象方法。
   - 抽象类不能实例化，只能被继承。
   - 接口（interface）是抽象的一种极端形式，所有方法默认是抽象的。
   - 抽象有助于建立良好的系统架构，强调“规范”而非“实现”。

2. 多态的实现方式

   **多态**（Polymorphism）指相同的接口或方法在不同对象上表现出不同的行为。分为 **编译时多态（方法重载）** 和 **运行时多态（方法重写）**。
